...
import { getUserSubscriptionPlan } from "../helpers/stripe.js";
...
const createPrice = asyncHandler(async (req, res, next) => {
  try {
    const orgId = req.params.orgId;
    const { amount, testId, productionId, subscriptionId } = req.body;
    ...
    res
      .status(201)
      .json({ message: "Price created successfully", status: true, newPrice });
  } catch (err) {
    next(err);
  }
});
